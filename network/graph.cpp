...
Graph::Graph(int Vertexes) {
    for (int i = 0; i < Vertexes; ++i)
        vertexSet.emplace_back(Vertex(i));
...
void Graph::addFlight(const int &src, const int &dest, const Airline &airline, double w) {
    vertexSet[src].addEdge(dest, airline, w);
}

void Graph::addAirport(const int &src, const Airport &airport) {
...
int Graph::cityFlights(const std::string &city) {
    int count = 0;
    for (int i = 0 ; i < getNumVertex(); i++)
        if (vertexSet[i].getAirport().getCity() == city)
            for (const Edge& e : vertexSet[i].getAdj())
                count++;
...
Graph::PairH Graph::airportsFromAirport(int source) const {
    ...
    for(const auto& e : vertexSet[source].getAdj())
        ans.insert({vertexSet[e.getDest()].airport.getCode(),
                    vertexSet[e.getDest()].airport.getName()});
    return ans;
}
...
unordered_set<string> Graph::countriesFromAirport(int i) const {
    ...
    for (const Edge& e : vertexSet[i].getAdj()){
        int w = e.dest;
        ans.insert(vertexSet[w].airport.getCountry());
    }
    return ans;
}
...
Vertex Graph::aStar(int src, int dest, Airline::AirlineH airlines) {
    ...
    while (!minHeap.empty()) {
        ...
        for (const auto &e : vertexSet[u].getAdj()) {
            ...
            if (!vertexSet[v].isVisited()) {
                double newDistance = vertexSet[u].distance + w;
                double heuristic = haversineDistance(vertexSet[v].getAirport().getLatitude(), vertexSet[v].getAirport().getLongitude(),
                                            vertexSet[dest].getAirport().getLatitude(), vertexSet[dest].getAirport().getLongitude());

                if (newDistance < vertexSet[v].distance) {
                    vertexSet[v].distance = newDistance;
                    vertexSet[v].parents = vertexSet[u].parents;
                    vertexSet[v].parents.push_back(v);
...
std::vector<std::pair<int, int>> Graph::maxDiameterSourceDest() {
    std::vector<std::pair<int, int>> maxDiameterVertices;
    ...
    for (int i = 0; i < getNumVertex(); ++i) {
        if (!vertexSet[i].isVisited()) {
            ...
            if (diameter > maxDiameter) {
                ...
                maxDiameterVertices.clear(); // Clear previous results
                maxDiameterVertices.emplace_back(i, -1); // Add potential source vertex
            }
    ...
    for (auto &pair : maxDiameterVertices) {
        int src = pair.first;
        if (!vertexSet[src].isVisited()) {
            vertexSet[src].setVisited(true);
            double diameter = bfsDiameter(src);
            for (int i = 0; i < getNumVertex(); ++i) {
                if (!vertexSet[i].isVisited() && bfsDiameter(i) == maxDiameter) {
                    pair.second = i; // Store potential destination vertex
                }
            }
        }
    }

    return maxDiameterVertices;
}
